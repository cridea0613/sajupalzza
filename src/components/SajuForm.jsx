...
import SavedReports from './SavedReports'; // 저장된 리포트 컴포넌트 임포트
...
const SajuForm = () => {
  ...
  const handleLoadReport = (reportData) => {
    ...
    setGender(reportData.gender);
    setTimeUnknown(reportData.timeUnknown);
    setYear(birthDate.getFullYear());
    setMonth(birthDate.getMonth() + 1);
    setDay(birthDate.getDate());
    
    if (reportData.timeUnknown) {
        setHour('');
        setMinute('');
    } else {
        setHour(birthDate.getHours());
  ...
  return (
    <StyledRoot>
      <Box sx={{ maxWidth: 500, width: '100%', mx: 'auto' }}>
        ...
        <SavedReports onLoadReport={handleLoadReport} />
      </Box>
    </StyledRoot>
  );
};
