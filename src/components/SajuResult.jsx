...
const StyledRoot = styled(Box)(({ theme }) => ({
  ...
  '@media print': {
    ...
    '& .no-print': { // 프린트 시 숨길 요소들에 적용할 클래스
      display: 'none',
    },
  },
}));
...
const TermModal = ({ open, handleClose, termData }) => { /* ... 이전과 동일 ... */ };
const ClickableTerm = ({ term, onClick }) => { /* ... 이전과 동일 ... */ };
const ContentRenderer = ({ content, onClickTerm }) => { /* ... 이전과 동일 ... */ };
const MonthlyTojeongBlock = ({ block, index }) => { /* ... 이전과 동일 ... */ };
    ...
    const renderBlock = (block, index) => {
        switch (block.type) {
            ...
            case 'tojeong_monthly':
                return <MonthlyTojeongBlock block={{ content: { monthly: [] } }} index={index} />;
            default:
                return null;
        }
    };
        ...
        <StyledRoot>
            <StyledPaper>
                ...
                <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, justifyContent: 'center', gap: 2, mt: 3 }} className="no-print">
                    <ActionButton variant="contained" size="large" startIcon={<ArrowBackIcon />} onClick={() => navigate('/')} color="secondary">
                        처음으로
                    </ActionButton>
                    <ActionButton variant="contained" size="large" startIcon={<ShareIcon />} onClick={handleShare}>
                        링크 공유
                    </ActionButton>
                    <ActionButton variant="contained" size="large" startIcon={<PictureAsPdfIcon />} onClick={handlePrint}>
                        PDF로 저장
                    </ActionButton>
                </Box>
